...
namespace CSharpSynth.Sequencer
{
    ...
    {
        // -- CUSTOM VARIABLE FOR TEMPO CHANGE
            ...
            if (_MidiFile.SequencerReady == false)
            {
                try
                {
                    ...
                    for (int x = 0; x < _MidiFile.Tracks[0].MidiEvents.Length; x++)
                    {
                        ...
                        {
                            _MidiFile.BeatsPerMinute = Convert.ToUInt32(MidiHelper.MicroSecondsPerMinute / System.Convert.ToDouble(_MidiFile.Tracks[0].MidiEvents[x].Parameters[0]) * Tempo);
                        }
                    }
